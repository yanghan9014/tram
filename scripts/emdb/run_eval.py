...
for root in emdb:
    ...
    seq = root.split('/')[-1]
    try:
        pred_cam = dict(np.load(f'{pred_dir}/camera/{seq}.npz'))
    except:
        print(f"Can't load {seq}, skipping...")
        continue
    ...
    results[seq] = re
    print(f"Results for sequence {seq}:")
    # print("  stats_slam:")
    # for key, value in re['stats_slam'].items():
    #     print(f"    {key}: {value}")
    # print("  stats_metric:")
    # for key, value in re['stats_metric'].items():
    #     print(f"    {key}: {value}")
...
df.to_excel(f"{args.pred_dir}/evaluation.xlsx", index=False)
